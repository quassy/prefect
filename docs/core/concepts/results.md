...
!!! tip "`NoResult` versus a `None` result"
...
- alternatively, you can provide a `location` [template which will be populated at runtime](/core/concepts/templating/)
...
To learn more about using results, check out the tutorial on [Using Results](../advanced_tutorials/using-results.html). If you used to use Result Handlers prior to 0.11.0, that tutorial also has a section on [Migrating from Result Handlers](/core/advanced_tutorials/using-results/#migrating-from-result-handlers)
